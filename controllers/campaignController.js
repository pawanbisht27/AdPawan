...
const { createDemoLeadsForCampaign } = require("./leadController");

exports.createCampaign = async (req, res) => {
  try {
    const { businessId, name, description, totalBudget, startDate, endDate } =
      req.body;

    if (!businessId || !name || !totalBudget || !startDate || !endDate) {
    ...
    if (Number.isNaN(parsedBudget) || parsedBudget <= 0) {
      return res.status(400).json({ message: "Invalid totalBudget" });
    }
...
exports.updateCampaign = async (req, res) => {
  try {
    const { name, description, totalBudget, startDate, endDate } = req.body;
    ...
    const finalEndDate = updateData.endDate || existingCampaign.endDate;
